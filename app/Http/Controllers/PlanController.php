...
class PlanController extends Controller
{
    ...
    public function subscribe(\Illuminate\Http\Request $request, Plan $plan)
    {
        if (Auth::user()->can('view-plans')) {
            ...
            $enabledGateways = PaymentGateway::enabled()->forTenant()->get()->map(function ($g) {
                return [
                    'name' => $g->name,
            ...
            // Fallback: if no gateway configured, provide bank transfer as default
            if (empty($enabledGateways)) {
                $enabledGateways = [[
                    'name' => 'bank_transfer',
                    'display_name' => __('Bank Transfer'),
                    'format' => 'bank_transfer',
                    'logo' => null,
                    'description' => __('Manual bank transfer payment'),
