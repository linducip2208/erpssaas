...
class HandleInertiaRequests extends Middleware
{
    ...
    public function share(Request $request): array
    {
        ...
        return [
            ...
            'auth' => [
                ...
                'lang' => $locale,
                'customMenus' => $this->getCustomMenus($request->user()),
            ],
            'flash' => [
    ...
    private function getCustomMenus($user): array
    {
        if (!$user) {
            return [];
        }

        if (!class_exists(CustomMenu::class) || !\Schema::hasTable('custom_menus')) {
            return [];
        }

        $tenantId = $user->type === 'superadmin' || $user->type === 'company'
            ? $user->id
            : $user->created_by;

        return CustomMenu::where('created_by', $tenantId)
            ...
            ->map(function ($menu) {
                return [
                    ...
                    'parent' => $menu->parent_id ? $menu->parent?->title : null,
